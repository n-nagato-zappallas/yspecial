...
if( !$self->get('release_date') ) {
	$now_date = date('Ymd') >= 20201218 ?date('Ymd') :'20201218';
...
if( $self->get('isp') == 70 ) {
	$SmartyObj->assign( 'uri_path',			( $device_mode == "sp" && !$real_flag ?'/sp' :'' ) .'/zap/event/getters_2021' );
	$SmartyObj->assign( 'img_path',			( $device_mode == "sp" && !$real_flag ?'/sp' :'' ) .'/zap/event/getters_2021' );
	$SmartyObj->assign( 'special_title',	'ゲッターズ飯田があなたの2021年を徹底鑑定◆五星三心占い運勢特集' );
	$SmartyObj->assign( 'meta_keywords',	'ゲッターズ飯田,五星三心,2021,運勢,無料,占い,当たる,突占,テレビ' );
	$SmartyObj->assign( 'meta_description',	'【無料で占えます】芸能界最強占い師ゲッターズ飯田があなたの2021年を丸ごと大鑑定！　無償で6万人以上を占ってきた実績をもとに編み出した「五星三心占い」で恋、仕事、お金や健康まで……2021年あなたに訪れる運命を徹底的に占います！' );
	$SmartyObj->assign( 'sns_description',	'【無料で占えます】芸能界最強占い師ゲッターズ飯田があなたの2021年を丸ごと大鑑定！　無償で6万人以上を占ってきた実績をもとに編み出した「五星三心占い」で恋、仕事、お金や健康まで……2021年あなたに訪れる運命を徹底的に占います！' );
	$SmartyObj->assign( 'special_period',	'2020年12月18日(金)～2021年12月23日(木)' );
	...
	$up = ( $device_mode == "sp" ?'/sp' :'' ) .'/rakuten/event/getters_2021';
	if( $real_flag ) $up = 'https://lovefortune.zappallas.com/rakuten/event/getters_2021';
	$SmartyObj->assign( 'uri_path',			$up );
	$SmartyObj->assign( 'img_path',			$up );
	$SmartyObj->assign( 'special_title',	'ゲッターズ飯田があなたの2021年を徹底鑑定◆五星三心占い運勢特集【楽天占い】' );
	$SmartyObj->assign( 'meta_keywords',	'ゲッターズ飯田,五星三心,2021,運勢,無料,占い,当たる,突占,テレビ' );
	$SmartyObj->assign( 'meta_description',	'【無料で占えます】芸能界最強占い師ゲッターズ飯田があなたの2021年を丸ごと大鑑定！　無償で6万人以上を占ってきた実績をもとに編み出した「五星三心占い」で恋、仕事、お金や健康まで……2021年あなたに訪れる運命を徹底的に占います！' );
	$SmartyObj->assign( 'sns_description',	'【無料で占えます】芸能界最強占い師ゲッターズ飯田があなたの2021年を丸ごと大鑑定！　無償で6万人以上を占ってきた実績をもとに編み出した「五星三心占い」で恋、仕事、お金や健康まで……2021年あなたに訪れる運命を徹底的に占います！' );
	$SmartyObj->assign( 'special_period',	'2020年12月18日(金)～2021年12月23日(木)' );
	...
	$up = ( $device_mode == "sp" ?'/sp' :'' ) .'/cp_zap/event/getters_2021';
	if( $real_flag ) $up = 'https://honkaku-uranai.jp/cp_zap/event/getters_2021';
	$SmartyObj->assign( 'uri_path',			$up );
	$SmartyObj->assign( 'img_path',			$up );
	$SmartyObj->assign( 'special_title',	'ゲッターズ飯田があなたの2021年を徹底鑑定◆五星三心占い運勢特集' );
	$SmartyObj->assign( 'meta_keywords',	'ゲッターズ飯田,五星三心,2021,運勢,無料,占い,当たる,突占,テレビ' );
...
//// 画像ぱす
// 'https://honkaku-uranai.jp/cp_zap/event/getters_2021'
//   ↓
// 'https://lw6nr4oyej.user-space.cdn.idcfcloud.net/event/getters_2021'
/** CDN使う場合 **/

$cdn_flag = true;
if( $cdn_flag && ( $self->get('isp') == 10 || $self->get('isp') == 11 ) ) {
	$img_path = 'https://lw6nr4oyej.user-space.cdn.idcfcloud.net/event/getters_2021';
	...
	if( ! $pack_flag && !$free_flag && ( $allfree_flag || $result_flag ) ) {
		...
		if( isset( $next ) ) {
			...
			foreach( $next as $next_id ) {
				...
				if( $self->get('menu_id') == 'gt21_001' && $data['menu_id'] == 'gt21_004' ) {
