...
if( !$self->get('release_date') ) {
	$now_date = date('Ymd') >= 20211220 ?date('Ymd') :'20211220';
...
if( $self->get('isp') == 70 ) {
	$SmartyObj->assign( 'uri_path',			( $device_mode == "sp" && !$real_flag ?'/sp' :'' ) .'/zap/event/getters_2022' );
	$SmartyObj->assign( 'img_path',			( $device_mode == "sp" && !$real_flag ?'/sp' :'' ) .'/zap/event/getters_2022' );
	$SmartyObj->assign( 'special_title',	'【ゲッターズ飯田の五星三心占い】あなたの2022年の全運勢特集' );
	$SmartyObj->assign( 'meta_keywords',	'ゲッターズ飯田,五星三心占い,2022年,運勢,無料,突然ですが占ってもいいですか' );
	$SmartyObj->assign( 'meta_description',	'【無料で占えます】あなたの2022年の運勢をゲッターズ飯田が占います！　総合運、仕事運、金運、恋愛運や結婚運まで、6万人以上を占ったデータをもとに編み出した『五星三心占い』で総力鑑定！　2022年を、あなたらしく最高な年にするために、まずは無料でお試しください。' );
	$SmartyObj->assign( 'sns_description',	'【無料で占えます】あなたの2022年の運勢をゲッターズ飯田が占います！　総合運、仕事運、金運、恋愛運や結婚運まで、6万人以上を占ったデータをもとに編み出した『五星三心占い』で総力鑑定！　2022年を、あなたらしく最高な年にするために、まずは無料でお試しください。' );
	$SmartyObj->assign( 'special_period',	'2021年12月20日(月)～2022年10月31日(月)' );
	$SmartyObj->assign( 'space_id', 	$device_mode == "sp" ?'2080534769' :'2080534768' );
	$SmartyObj->assign( 'space_id_tag',	$device_mode == "sp" ?'<SPACE_ID_MEM_S>2080534769<SPACE_ID_MEM_E>' :'<SPACE_ID_EMG_S>2080534768<SPACE_ID_EMG_E>' );

} else if( $self->get('isp') == 10 ) {
	$up = ( $device_mode == "sp" ?'/sp' :'' ) .'/rakuten/event/getters_2022';
	if( $real_flag ) $up = 'https://lovefortune.zappallas.com/rakuten/event/getters_2022';
	$SmartyObj->assign( 'uri_path',			$up );
	$SmartyObj->assign( 'img_path',			$up );
	$SmartyObj->assign( 'special_title',	'【ゲッターズ飯田の五星三心占い】あなたの2022年の全運勢特集【楽天占い】' );
	$SmartyObj->assign( 'meta_keywords',	'ゲッターズ飯田,五星三心占い,2022年,運勢,無料,突然ですが占ってもいいですか' );
	$SmartyObj->assign( 'meta_description',	'【無料で占えます】あなたの2022年の運勢をゲッターズ飯田が占います！　総合運、仕事運、金運、恋愛運や結婚運まで、6万人以上を占ったデータをもとに編み出した『五星三心占い』で総力鑑定！　2022年を、あなたらしく最高な年にするために、まずは無料でお試しください。' );
	$SmartyObj->assign( 'sns_description',	'【無料で占えます】あなたの2022年の運勢をゲッターズ飯田が占います！　総合運、仕事運、金運、恋愛運や結婚運まで、6万人以上を占ったデータをもとに編み出した『五星三心占い』で総力鑑定！　2022年を、あなたらしく最高な年にするために、まずは無料でお試しください。' );
	$SmartyObj->assign( 'special_period',	'2021年12月20日(月)～2022年12月22日(木)' );

} else if( $self->get('isp') == 11 ) {
	$up = ( $device_mode == "sp" ?'/sp' :'' ) .'/cp_zap/getters_2022';
	if( $real_flag ) $up = 'https://honkaku-uranai.jp/cp_zap/getters_2022';
	$SmartyObj->assign( 'uri_path',			$up );
	$SmartyObj->assign( 'img_path',			$up );
	$SmartyObj->assign( 'special_title',	'【ゲッターズ飯田の五星三心占い】あなたの2022年の全運勢特集' );
	$SmartyObj->assign( 'meta_keywords',	'ゲッターズ飯田,五星三心占い,2022年,運勢,無料,突然ですが占ってもいいですか' );
	$SmartyObj->assign( 'meta_description',	'あなたの2022年の運勢をゲッターズ飯田が占います！　総合運、仕事運、金運、恋愛運や結婚運まで、6万人以上を占ったデータをもとに編み出した『五星三心占い』で総力鑑定！　2022年を、あなたらしく最高な年にするために、まずは無料でお試しください。' );
	$SmartyObj->assign( 'sns_description',	'あなたの2022年の運勢をゲッターズ飯田が占います！　総合運、仕事運、金運、恋愛運や結婚運まで、6万人以上を占ったデータをもとに編み出した『五星三心占い』で総力鑑定！　2022年を、あなたらしく最高な年にするために、まずは無料でお試しください。' );
	$SmartyObj->assign( 'special_period',	'2021年12月20日(月)～2022年12月22日(木)' );

}

//// 画像ぱす
// 'https://honkaku-uranai.jp/cp_zap/event/getters_2022'
//   ↓
// 'https://lw6nr4oyej.user-space.cdn.idcfcloud.net/event/getters_2022'
/** CDN使う場合 **/

$cdn_flag = true;
//$cdn_flag = false;
if( $cdn_flag && ( $self->get('isp') == 10 || $self->get('isp') == 11 ) ) {
	$img_path = 'https://lw6nr4oyej.user-space.cdn.idcfcloud.net/event/getters_2022';
	...
	if( ! $pack_flag && !$free_flag && ( $allfree_flag || $result_flag ) ) {
		...
		if( isset( $next ) ) {
			...
			foreach( $next as $next_id ) {
				...
				if( $self->get('menu_id') == 'gs22_001' && $data['menu_id'] == 'gs22_004' ) {
