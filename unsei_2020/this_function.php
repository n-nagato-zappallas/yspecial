...
function getIndexData3( $arr = array() ) {
	...
	foreach( $arr as $menu_id => $row ) {
		...
		if( $self->get('isp') == 11 ) {
			if( ( $menu_id == 219 || $menu_id == 903 || $menu_id == 309 || $menu_id == 501 ) && 20200109 > $release_date ) continue;
			if( $menu_id == 901 && 20200123 > $release_date ) continue;
		}
		...
		foreach( $row as $key => $val ) {
			...
			if( $self->get('isp') == 10 ) {
				...
				if( strpos( $key, 'price' ) !== false ) {
					...
					$data['discount_flag'] = $menu_id=='202' ?0 :1;
